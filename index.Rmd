...
# 
# # Update baseline_adoption from data_3 (Adoption WITHOUT VBA)
# data$lower[data$variable == "baseline_adoption"] <- round(quantile(as.numeric(data_3$`Adoption WITHOUT VBA (%)`), 0.1, na.rm = TRUE)/100, 2)
# data$upper[data$variable == "baseline_adoption"] <- round(quantile(as.numeric(data_3$`Adoption WITHOUT VBA (%)`), 0.9, na.rm = TRUE)/100, 2)
# 
# # Update vba_effectiveness from data_3 (difference between with/without VBA)
# vba_effect <- (data_3$`Adoption WITH VBA Support (%)` - as.numeric(data_3$`Adoption WITHOUT VBA (%)`))/100
...
# 
# # Update base_yield from data_4
# data$lower[data$variable == "base_yield"] <- round(quantile(as.numeric(data_4$`Baseline (bags/ac)`), 0.1, na.rm = TRUE) * 0.025, 1)
# data$upper[data$variable == "base_yield"] <- round(quantile(as.numeric(data_4$`Baseline (bags/ac)`), 0.9, na.rm = TRUE) * 0.025, 1)
# 
# # Update ra_adoption_cost from data_5 (labor + inputs)
# total_cost <- data_5$`Total  Labour Cost (KSh)` + data_5$`Total  Inputs Cost (KSh)`
...
# 
# # Update yield_impact_ra from data_4 (yield improvement)
# yield_improvement <- (as.numeric(data_4$`Highest Yield in bags/ac`) - as.numeric(data_4$`Baseline (bags/ac)`)) / as.numeric(data_4$`Baseline (bags/ac)`)
...
simulation_results <- mcSimulation(
  estimate = estimate_read_csv("data/updated_inputs_vba_kenya.csv"),
